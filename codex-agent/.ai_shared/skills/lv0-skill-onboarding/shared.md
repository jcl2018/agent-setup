...
Use this {{WRAPPER_NOUN}} when the job is to create, rename, extend, or standardize shared skills and agents inside this home-agent mirror. It turns authoring into a repeatable workflow instead of a memory exercise and keeps the `lv0` / `lv1` / `lv2` split explicit.
...
6. Treat `{{HOME_TOOL_DIR}}` as generated wrapper output for shared skills. Only hand-edit the tool wrapper directly when the change is truly Codex-specific and cannot live in `.ai_shared/skills/`.
