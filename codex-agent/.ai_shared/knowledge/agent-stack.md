...
## Purpose
Give the home-root AI setup one shared routing map that Codex can use without duplicating the same context twice.

## Layer Order
1. Tool entry wrappers
   - home entrypoints such as `~/.codex/AGENTS.md`
   - repo-local tool overrides such as `.codex/` when a repo needs tool-specific behavior
...
- `~/.codex/`: tool-native home wrappers, config, and skills
- `.codex/`: repo-local tool overrides and thin wrappers when a repo needs tool-specific behavior
...
- New repo-specific wrapper: tool wrapper + `lv0-instruction-core` + `lv0-run-transparency` + one `lv0` specialist + one repo-specific `lv2-<repo>-<name>` override in `.codex/`
