...
- Keep tool runtime and session state local: files such as `.codex/auth.json`, `.codex/sessions/`, `.codex/sqlite/`, `.codex/tmp/`, and similar runtime-only surfaces.
...
- When a path must point to a local home install surface, use home-relative forms such as `~/.ai_shared/` or `~/.codex/` instead of a user-specific absolute path.
