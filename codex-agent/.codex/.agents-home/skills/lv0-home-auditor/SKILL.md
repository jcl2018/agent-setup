...
11. When the audit results are being printed, run `scripts/checkup-home-state.ps1 -RepoRoot <path> -HomeRoot <path>` from this skill so the output includes a `Checkup` stage listing the current machine's managed skills, rules, and agent surfaces plus any repo-to-home discrepancies. Ignore known unmanaged tool-native extras such as Codex `.system` unless a repo-specific rule says otherwise.
