...
## Purpose
Verify that the shared home-root AI setup and the triggering repo's local shared folders remain aligned before deeper work continues.
...
- current date and `~/.ai_shared/knowledge/progress-tracker.md`
...
## Steps
1. Check whether `~/.ai_shared/knowledge/progress-tracker.md` already logged an `lv0-home-auditor` run during the current calendar week.
...
6. Check repo-specific rules, continuity docs, and local overrides for the repo that triggered the weekly pass.
7. Update `~/.ai_shared/knowledge/progress-tracker.md` with the audit result and move unfinished cleanup work into `~/.ai_shared/knowledge/future-plan.md`.
