...
Use this skill as the weekly `lv0` auditor for home-folder updates. On the first repo request handled anywhere during a new calendar week, verify that the shared local config, folder structure, tool wrappers, and the triggering repo's local rules are still aligned before deeper work continues.
