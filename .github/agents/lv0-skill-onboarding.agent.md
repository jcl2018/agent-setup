...
Use this agent when the job is to create, rename, extend, or standardize shared skills and agents inside this home-agent mirror. It turns authoring into a repeatable workflow instead of a memory exercise.
...
6. Treat `~/.github/` as generated wrapper output for shared skills. Only hand-edit the tool wrapper directly when the change is truly tool-specific and cannot live in `.ai_shared/skills/`.
