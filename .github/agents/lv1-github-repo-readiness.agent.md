...
Use this shared `lv1` agent when a repository needs GitHub-facing cleanup, private handoff hardening, or public-release preparation.
...
6. Run `scripts/audit-repo.ps1 -RepoPath <path> -Visibility <private|public|either>` from this agent when PowerShell is available. If PowerShell is unavailable, mirror the same checks manually.
