...
Use this agent as the weekly `lv0` auditor for home-folder updates. On the first request in a repo during a new calendar week, verify that the shared local config, folder structure, tool wrappers, and repo-specific rules are still aligned before deeper work continues.
