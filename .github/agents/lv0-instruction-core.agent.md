...
6. Use `~/.ai_shared/workflows/`, `~/.ai_shared/templates/`, `~/.ai_shared/checklists/`, and `~/.ai_shared/knowledge/` as shared defaults, and keep `~/.github/` for Copilot-native wrapper logic, config, and agents and instructions.
...
9. When creating a repo-specific Copilot wrapper, keep it thin: reference this agent first, add the smallest relevant specialist or task wrapper, then add only Copilot-specific behavior that the shared `.ai_shared/` layer cannot express.
