...
- Changed: updated the feature-dev skill, feature workflow, feature-validation checklist, top-level routing docs, and agent-stack notes across `.codex`, `.claude`, and `.github`; added a reusable `prd-template.md` to each tool root so feature work can create or update PRDs alongside implementation.
...
- Changed: added `lv1-figma-implement-design` and `lv1-github-repo-readiness` under the shared skill stack, copied the reusable GitHub checklist and audit script into the Claude skill tree, broadened the GitHub readiness guidance so it supports both private and public repos, and updated the shared stack and discovery docs across `.codex`, `.claude`, `.github`, and the root `AGENTS.md`.
