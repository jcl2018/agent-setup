...
2. Read the most relevant repo-local context from `.claude/knowledge/`, especially `coding-standards.md`, `naming-conventions.md`, `repo-map.md`, and `test-commands.md`. Use `~/.claude/knowledge/` only for shared cross-repo guidance when the repo has no local equivalent.
