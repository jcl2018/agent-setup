...
3. Treat `~/.claude/knowledge/progress-tracker.md` and `~/.claude/knowledge/future-plan.md` as required repo docs. Create them if they are missing, record completed context in the progress tracker, and capture unfinished work or follow-up todos in the future plan.
4. Use `~/.claude/workflows/` for process, `~/.claude/knowledge/` for durable facts, `~/.claude/templates/` for structured outputs, and `~/.claude/checklists/` for wrap-up validation.
